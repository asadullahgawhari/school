...
class HomeworkSubmitModel extends Model
{
    ...
    static public function getHomeworkReport()
    {
        $return = self::select('homework_submit.*', 'class.name as class_name', 'subject.name as subject_name', 'users.name as first_name', 'users.last_name')
            ...
            {
                $return = $return->where('homework_submit.created_at', '>=', Request::get('from_created_at'));        
            }
            if (!empty(Request::get('to_created_at'))) 
            {
                $return = $return->where('homework_submit.created_at', '<=', Request::get('to_created_at'));        
            }
    ...
    // Count Submitted Homwork for student
    ...
    {
        if (!empty($this->document_file) && file_exists('public/dist/img/homework/'.$this->document_file)) 
        {
            return url('public/dist/img/homework/'.$this->document_file);
        }
        else
        {
            return "";
